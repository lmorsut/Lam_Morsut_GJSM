...
#Self-Cutoff
ENDMCS=300000 #call runtime here directly
#Mitosis Variables
...
MTFORCEMIN=0 #negative mitosis driving force fluctuation, usually only need to change the exponential part
MTFORCEMAX=0  #positive mitosis driving force fluctuation, usually change only the exponential part
#Signaling Variables
...
BETAYG=-921.181/10 #threshold of signal required to generate a response in your cell due to signaling
EPSILONYG=526.389/10 #modulates how sharp the response is due to signaling, can turn synthesis to linear or heavi-side theta like if desired
KAPPAYG=25000 #general decay constant
THRESHOLDUPYG=7000 #activation threshold to change state  
THRESHOLDDOYG=3500 #deactivation threshold to revert state, as no clear deactivation/unsorting occured in reference experiments

ALPHABR=1 #controls how much your reporter synthesis magnitude due to signal S; can be set to 1 if decay is set properly
BETABR=-921.181/10 #threshold of signal required to generate a response in your cell due to signaling
EPSILONBR=526.389/10 #modulates how sharp the response is due to signaling, can turn synthesis to linear or heavi-side theta like if desired
KAPPABR=25000 #general decay constant
THRESHOLDUPBR=7000 #activation threshold to change state, the choice of paramters in this paragraph render it similar to that of the previous paragraph
THRESHOLDDOBR=3500 #deactivation threshold to revert state, as no clear deactivation/unsorting occured in reference experiments
...
#                 print CSAY, CSAG, PTSG, CSAB, PTSB, CSAR, PTSR, CSAM
            ...
            if (cell.type==2):
                #DTRES=GENEXPSCF*math.exp(-((PTSR+PTSB-VARSIGAVG)/VARSIGWID)**2)-(1/DECAYRATE)*cell.dict["PTS"][0]
                DTRES=(1/(ALPHAYG+math.exp(-((PTSR+PTSB)-BETAYG)/EPSILONYG)))-(1/KAPPAYG)*cell.dict["PTS"][0]
                cell.dict["PTS"][0]+=DTRES                
            if (cell.type==4):
                #DTRES=GENEXPSCF*math.exp(-((PTSY+PTSG-VARSIGAVG)/VARSIGWID)**2)-(1/DECAYRATE)*cell.dict["PTS"][0]
                DTRES=(1/(ALPHABR+math.exp(-((PTSG+PTSY)-BETABR)/EPSILONBR)))-(1/KAPPABR)*cell.dict["PTS"][0]
                cell.dict["PTS"][0]+=DTRES
            if (cell.type==1):
                #DTRES=GENEXPSCF*math.exp(-((PTSR+PTSB-VARSIGAVG)/VARSIGWID)**2)-(1/DECAYRATE)*cell.dict["PTS"][0]
                DTRES=(1/(ALPHAYG+math.exp(-((0)-BETAYG)/EPSILONYG)))-(1/KAPPAYG)*cell.dict["PTS"][0]
                cell.dict["PTS"][0]+=DTRES                
            if (cell.type==3):
                #DTRES=GENEXPSCF*math.exp(-((PTSY+PTSG-VARSIGAVG)/VARSIGWID)**2)-(1/DECAYRATE)*cell.dict["PTS"][0]
                DTRES=(1/(ALPHABR+math.exp(-((0)-BETABR)/EPSILONBR)))-(1/KAPPABR)*cell.dict["PTS"][0]
                cell.dict["PTS"][0]+=DTRES                 
            ...
            if cell.type==1: #gray cells
                ...
                if (CSAY+CSAG)>0:                 # count the number of Yg cells in contact with YG cells
                    YGCYG+=1                    
                cell.lambdaSurface=2.0            #change depending on cell adhesitivity
                cell.lambdaVolume=2.0             #change depending on cell adhesitivity  
