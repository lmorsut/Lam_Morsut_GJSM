...
class ELUGMSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.pW7 = self.addNewPlotWindow(
            _title='Peri',
            _xAxisTitle='MonteCarlo Step (MCS)',
            _yAxisTitle='Count',
            _xScaleType='linear',
            _yScaleType='linear',
            _grid=True)                
        self.pW7.addPlot('PR', _style='Dots', _color='red', _size=3)
        self.pW7.addPlot('PG', _style='Dots', _color='green', _size=3)  #bright field and merged field sphericity    
        ...
        for cell in self.cellList: #iterate over cell list
            ...
            for neighbor, commonSurfaceArea in self.getCellNeighborDataList(cell): #iterate for each cell its neighbors
                ...
                if neighbor.type==2: #green cells
                    ...
                    PTSG+=0 #PHI and L as in the text
...
#                 print CSAY, CSAG, PTSG, CSAB, PTSB, CSAR, PTSR, CSAM
            ...
            if (cell.type==2 or cell.type==4): #which cells receive what type of signal
...
#vetting code
        ...
        if mcs%RESOL==0:
            ...
            if NUMTR==0:
                self.pW7.addDataPoint("PR", mcs, 0)
            if NUMTR>0:
                self.pW7.addDataPoint("PR", mcs, PERIR/NUMTR)            
            if NUMTG==0:
                self.pW7.addDataPoint("PG", mcs, 0)
            if NUMTG>0:
                self.pW7.addDataPoint("PG", mcs, PERIG/NUMTG)

            if (NUMTY+NUMTG)==0:
                self.pW4.addDataPoint("YG", mcs, 0)
            if (NUMTY+NUMTG)>0:
                self.pW4.addDataPoint("YG", mcs, YGPTS/(NUMTY+NUMTG))
            if (NUMTB+NUMTR)==0:
                self.pW4.addDataPoint("BR", mcs, 0)
            if (NUMTB+NUMTR)>0:
            ...
            if mcs==ENDMCS:
                ...
                fileName = "Surface" + str(mcs) + ".txt"
                self.pW7.savePlotAsData(fileName)                 
                self.stopSimulation()                 
    def finish(self):
        pass
